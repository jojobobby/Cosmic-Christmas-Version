...
public class TextureRedrawer {
    ...
    public static const OUTLINE_FILTER:GlowFilter = new GlowFilter(0, 1, 1.2, 1.2, 0xFF, Parameters.data_.bitmapQuality, false, false);
    public static const STANDARD_OUTLINE_FILTER:Array = [new GlowFilter(0, 1, 2, 2, 10, 1)];
    ...
    public static function redraw(tex:BitmapData, size:int, padBottom:Boolean, glowColor:uint, useCache:Boolean = true, sMult:Number = 5, oSize:Number = 1.4):BitmapData {
        ...
        modTex = GlowRedrawer.outlineGlow(modTex, glowColor, oSize, useCache);
        if (debug) {
            modTex = drawDebugBorder(modTex);
        }
        if (useCache) {
    ...
    private static function drawDebugBorder(tex:BitmapData):BitmapData {
        var shape:Shape = new Shape();
        var g:Graphics = shape.graphics;
        g.lineStyle(1,Math.random() * 0xFFFFFF,1,false,LineScaleMode.NORMAL,CapsStyle.ROUND,JointStyle.ROUND);
        g.drawRect(0,0,tex.width - 2,tex.height - 2);
        tex.draw(shape);
        return tex;
    }
