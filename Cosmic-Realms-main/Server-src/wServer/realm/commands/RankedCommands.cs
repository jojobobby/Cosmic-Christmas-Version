...
namespace wServer.realm.commands
{
    ...
    {
        public DayCommand() : base("Day", permLevel: 0) { }
    ...
    {
        public NightCommand() : base("Night", permLevel: 0) { }
    ...
    class SpawnCommand : Command
    {
        ...
        private bool SpawnJson(Player player, string json)
        {
            ...
            if (props.spawns != null)
            {
                foreach (var spawn in props.spawns)
                {
                    ...
                    if (player.Client.Account.Rank < 100 && player.Owner is DeathArena && desc.ObjectId.Contains("Fountain"))
                    {
                    ...
                    {
                        player.SendError("You can only spawn one enemy.");
                    }
        ...
        private bool SpawnBasic(Player player, string args)
        {
            ...
            {
                player.Client.Account.OneTimeSpawn = false;
        ...
        private void NotifySpawn(Player player, string mob, int? num = null)
        {
            ...
            var notif = mob;
            if (num != null)
                notif = player.Name + " is spawning a " + ((num > 1) ? num + " " : "") + mob + "...";
        ...
        public void QueueSpawnEvent(Player player, int num, ushort mobObjectType, int? hp = null, int? size = null, int[] x = null, int[] y = null, bool? target = false)
        {
            var pX = player.X;
            var pY = player.Y;
